...
class AIApiService {
    ...
    private async generateTitleFromMessage(message: string) {
        ...
        const result = await ai.models.generateContent({
            ...
            contents: `Сообщения: //// \`${message}\` ////. Сгенерируй короткий заголовок который подойдет для названия чата (максимум 4-6 слов), который описывает это сообщение. Только один заголовок и больше ничего`
        })
